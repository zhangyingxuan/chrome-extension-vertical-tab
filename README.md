...
1. **切换排序方式**：在底部工具栏选择排序类型
2. **拖拽排序**：在分组内或未分组区域拖拽标签页进行排序
3. **自动同步**：排序结果会自动同步到 Chrome 标签页顺序
...
2. **拖拽系统**：实现标签页的拖拽分配和排序功能
3. **存储系统**：使用 Chrome Storage API 保存用户偏好
4. **事件监听**：监听 Chrome 标签页和分组的变化事件
5. **搜索功能**：实现高效的标签页搜索和过滤
